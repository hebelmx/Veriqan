#!/usr/bin/env python3
"""
Extraction Test Duplicate Detector for Prisma
Focuses on the 4 extraction test projects where "confused agent" caused duplication:
1. Tests.Infrastructure.Extraction
2. Tests.Infrastructure.Extraction.GotOcr2
3. Tests.Infrastructure.Extraction.Teseract
4. Tests.System
"""
...
class ExtractionTestDuplicateDetector:
    """Detects duplicate test methods in extraction test projects."""
    ...
    def scan_test_methods(self) -> Dict[str, List[Dict]]:
        """Scan ONLY the 4 target extraction test projects."""
        all_methods = {}

        print(f"Scanning extraction test files in: {', '.join(self.TARGET_PROJECTS)}")

        # Scan only the 4 target projects
        test_files = []
        for project in self.TARGET_PROJECTS:
            project_path = self.base_path / project
            if project_path.exists():
                test_files.extend(list(project_path.rglob("*Tests.cs")))
                print(f"  Found {len(list(project_path.rglob('*Tests.cs')))} test files in {project}")

        print(f"Total: {len(test_files)} test files across 4 projects")
    ...
    def detect_duplicates(self) -> Dict:
        """Detect duplicate test methods in extraction test projects."""
        print("Starting extraction test duplicate detection...\n")
        all_methods = self.scan_test_methods()

        analysis = {
            'exact_duplicates': [],
            'cross_project_duplicates': [],  # CRITICAL: Same test in different projects
            'same_project_duplicates': [],    # Same test duplicated within one project
        ...
        for normalized_name, instances in all_methods.items():
            if len(instances) > 1:
                # Check if duplicates are across different projects
                projects = set(inst['project'] for inst in instances)
                ...
                duplicate_entry = {
                    ...
                    'crossProject': is_cross_project,
                    'recommendation': '🔥 CRITICAL - Cross-project duplication!' if is_cross_project else '⚠️  REVIEW - Same method name in multiple locations'
                }

                analysis['exact_duplicates'].append(duplicate_entry)
                analysis['statistics']['exact_duplicates'] += 1

                if is_cross_project:
                    analysis['cross_project_duplicates'].append(duplicate_entry)
                    analysis['statistics']['cross_project'] += 1
                else:
                    analysis['same_project_duplicates'].append(duplicate_entry)
                    analysis['statistics']['same_project'] += 1

        # Find similar method names (fuzzy matching)
        processed_pairs = set()
        method_list = list(all_methods.items())

        print(f"\nComparing {len(method_list)} methods for similarity...")
    ...
    def generate_report(self, analysis: Dict, output_file: str):
        ...
        analysis['metadata'] = {
            ...
            'analyzer_version': '1.0.0_prisma_extraction_tests',
            'scope': f'4 extraction test projects: {", ".join(self.TARGET_PROJECTS)}',
            'thresholds': {
                'exact_match': 1.0,
        ...
        print(f"\nExtraction test duplicate analysis saved to: {output_file}")
        return analysis

    def print_summary(self, analysis: Dict):
        """Print summary of extraction test duplicate detection."""
        print("\n" + "="*80)
        print("PRISMA EXTRACTION TEST DUPLICATE DETECTION")
        print("Target Projects: Tests.Infrastructure.Extraction, GotOcr2, Teseract, System")
        print("="*80)
        ...
        print(f"   🔥 Cross-Project Duplicates: {stats['cross_project']}")
        print(f"   ⚠️  Same-Project Duplicates: {stats['same_project']}")
        print(f"   🔄 High Similarity: {stats['high_similarity']}")
        print(f"   ❓ Suspicious Similarities: {stats['suspicious']}")

        if analysis['cross_project_duplicates']:
            print(f"\n{'='*80}")
            print(f"🔥 CRITICAL: CROSS-PROJECT DUPLICATES (likely from confused agent)")
            print(f"{'='*80}")
            for dup in analysis['cross_project_duplicates']:
                print(f"\n🔴 {dup['normalizedName']}")
                print(f"   Instances: {dup['instanceCount']} across {len(dup['projects'])} projects")
                print(f"   Projects: {', '.join(dup['projects'])}")
                print(f"   Occurrences:")
                for inst in dup['instances']:
                    print(f"      • {inst['className']}.{inst['originalName']} in {inst['project']}")
                    print(f"        {inst['fileName']}")

        if analysis['same_project_duplicates']:
            print(f"\n{'='*80}")
            print(f"⚠️  SAME-PROJECT DUPLICATES")
            print(f"{'='*80}")
            for dup in analysis['same_project_duplicates'][:10]:
                print(f"\n🟡 {dup['normalizedName']}")
                print(f"   Instances: {dup['instanceCount']} in {dup['projects'][0]}")
                for inst in dup['instances'][:5]:
                    print(f"      • {inst['className']}.{inst['originalName']}")
                    print(f"        {inst['fileName']}")

        if analysis['high_similarity_duplicates']:
            print(f"\n{'='*80}")
            print(f"🔄 HIGH SIMILARITY PAIRS (may indicate copy-paste errors)")
            print(f"{'='*80}")
            for sim in analysis['high_similarity_duplicates'][:15]:
                print(f"\n🟠 {sim['method1']} ≈ {sim['method2']}")
                print(f"   Similarity: {sim['similarity']:.2%}")
                print(f"   Method 1: {len(sim['instances1'])} instances in {set(i['project'] for i in sim['instances1'])}")
                print(f"   Method 2: {len(sim['instances2'])} instances in {set(i['project'] for i in sim['instances2'])}")
...
def main():
    ...
    detector = ExtractionTestDuplicateDetector(base_path)
    analysis = detector.detect_duplicates()

    # Generate output filename
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"extraction_test_duplicate_analysis_{timestamp}.json"
