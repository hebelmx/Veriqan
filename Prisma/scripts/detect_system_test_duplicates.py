#!/usr/bin/env python3
"""
System Test Duplicate Detector for Prisma
Focuses ONLY on Tests.System* projects to find duplicates
Ignores Infrastructure test duplication (which is EXPECTED for Liskov verification)
"""
...
class SystemTestDuplicateDetector:
    """Detects duplicate test methods ONLY in System test projects."""
    ...
    def scan_test_methods(self) -> Dict[str, List[Dict]]:
        """Scan ONLY System test files for test methods."""
        all_methods = {}

        print(f"Scanning System test files in {self.base_path}...")

        # ONLY scan Tests.System* projects using rglob
        test_files = []
        for test_file in self.base_path.rglob("*Tests.cs"):
            # Filter to only include Tests.System* projects
            if "Tests.System" in str(test_file):
                test_files.append(test_file)

        print(f"Found {len(test_files)} System test files")
    ...
    def detect_duplicates(self) -> Dict:
        """Detect duplicate test methods in System test projects."""
        print("Starting System test duplicate detection...")
        ...
        analysis = {
            ...
            'cross_project_duplicates': [],  # NEW: Track duplicates across different System projects
            'statistics': {
        ...
        for normalized_name, instances in all_methods.items():
            if len(instances) > 1:
                ...
                duplicate_entry = {
                    ...
                    'recommendation': 'CRITICAL - Cross-project duplication!' if is_cross_project else 'REVIEW - Same method name in multiple locations'
                }
    ...
    def generate_report(self, analysis: Dict, output_file: str):
        ...
        analysis['metadata'] = {
            ...
            'analyzer_version': '1.0.0_prisma_system_tests',
            'scope': 'Tests.System* projects ONLY',
            'thresholds': {
        ...
        print(f"\nSystem test duplicate analysis saved to: {output_file}")
        return analysis

    def print_summary(self, analysis: Dict):
        """Print summary of System test duplicate detection."""
        print("\n" + "="*70)
        print("PRISMA SYSTEM TEST DUPLICATE DETECTION")
        print("="*70)

        stats = analysis['statistics']
        print(f"📊 Total System Test Methods: {stats['total_methods']}")
        ...
        if analysis['cross_project_duplicates']:
            print(f"\n🔥 CRITICAL: CROSS-PROJECT DUPLICATES:")
            for dup in analysis['cross_project_duplicates']:
                print(f"\n   🔴 {dup['normalizedName']} ({dup['instanceCount']} instances across {len(dup['projects'])} projects)")
        ...
        if analysis['exact_duplicates']:
            ...
            for dup in analysis['exact_duplicates'][:10]:
                if not dup['crossProject']:  # Show non-cross-project duplicates
...
def main():
    ...
    detector = SystemTestDuplicateDetector(base_path)
    analysis = detector.detect_duplicates()

    # Generate output filename
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"system_test_duplicate_analysis_{timestamp}.json"
