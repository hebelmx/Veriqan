#!/usr/bin/env python3
"""
xUnit1026 XUnitLogger Pattern Fixer V5 - ASYNC TASK SUPPORT
Handles both public void and public async Task method patterns
"""
...
class AsyncTaskXUnitLoggerFixer:
    ...
    def find_method_opening_brace_by_name_with_async(self, lines: List[str], method_name: str) -> int:
        """
        V5 IMPROVED: Find opening brace supporting both public void and public async Task patterns
        """
        brace_line = -1
        method_line = -1
        
        print(f"    DEBUG: Searching for method '{method_name}' (with async support)")
        
        # Search for both patterns: public void and public async Task
        patterns = [
            f'public void {method_name}(',
            f'public async Task {method_name}('
        ]
        
        # First find the method by name with either pattern
        for i, line in enumerate(lines):
            for pattern in patterns:
                if pattern in line:
                    method_line = i
                    print(f"    DEBUG: Found method declaration on line {i + 1} with pattern: {pattern}")
                    break
            if method_line != -1:
                break
        
        if method_line == -1:
            print(f"    DEBUG: Could not find method '{method_name}' with either pattern")
            return -1
    ...
    def fix_single_method(self, file_path: str, error_info: Dict) -> bool:
        """Fix a single method - V5 with async Task support"""
        try:
            ...
            # V5 IMPROVED: Better brace detection by method name with async support
            method_start_brace = self.find_method_opening_brace_by_name_with_async(lines, error_info['method'])
            ...
            patterns = [
                f'public void {error_info["method"]}(',
                f'public async Task {error_info["method"]}('
            ]
    ...
    def run(self, target_dir: str):
        """Main execution - V5 with async Task support"""
        print("XUnitLogger Pattern Fixer V5 - ASYNC TASK SUPPORT")
        ...
        # Final summary
        print("\n" + "="*60)
        print("V5 ASYNC TASK SUPPORT FIXER SUMMARY")
        print("="*60)
...
def main():
    parser = argparse.ArgumentParser(description='V5 Async Task Support XUnitLogger Fixer')
    ...
    fixer = AsyncTaskXUnitLoggerFixer(dry_run=args.dry_run, max_files=args.max_files)
