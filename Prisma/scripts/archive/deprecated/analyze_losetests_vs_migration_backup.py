#!/usr/bin/env python3
"""
Analyze Losetests Directory vs Migration Backup
================================================

PURPOSE: Compares remaining Losetests files against migration_backup/ExxerAI.IntegrationTests

SAFETY: Read-only analysis - makes NO modifications to files.

USAGE:
    python scripts/analyze_losetests_vs_migration_backup.py --base-path "F:/Dynamic/ExxerAi/ExxerAI"

OUTPUT:
    - Console report showing duplicates, unique files, and similar files
    - JSON report: losetests_vs_migration_backup.json
"""
...
class FileComparison:
    """Represents a comparison between a Losetests file and a migration backup file."""
    losetests_file: str
...
class MigrationBackupAnalyzer:
    """Analyzes Losetests directory against migration backup."""

    def __init__(self, base_path: str):
        self.base_path = Path(base_path)
        self.losetests_dir = self.base_path / "code" / "src" / "Losetests"
        self.migration_backup_dir = self.base_path / "migration_backup" / "ExxerAI.IntegrationTests"
    ...
    def find_matching_files(self, filename: str) -> List[Path]:
        """Find all files with the same name in the migration backup directory."""
        matching_files = []

        if not self.migration_backup_dir.exists():
            print(f"⚠️  Migration backup directory not found: {self.migration_backup_dir}")
            return matching_files

        for backup_file in self.migration_backup_dir.rglob(filename):
    ...
    def compare_file(self, losetests_file: Path) -> FileComparison:
        """Compare a Losetests file against the migration backup directory."""
    ...
    def analyze(self) -> Dict:
        """Perform full analysis of Losetests directory vs migration backup."""
        print("\n" + "="*80)
        print("🔍 LOSETESTS VS MIGRATION BACKUP ANALYSIS")
        print("="*80)
        ...
        if not self.migration_backup_dir.exists():
            print(f"\n❌ ERROR: Migration backup directory not found: {self.migration_backup_dir}")
            return {}

        print(f"\n📂 Scanning: {self.losetests_dir}")
        print(f"📂 Comparing against: {self.migration_backup_dir}")
        ...
        summary = {
            ...
            'migration_backup_directory': str(self.migration_backup_dir.relative_to(self.base_path))
        }
    ...
    def print_report(self, report: Dict):
        ...
        if report['similar_files']:
            print("\n" + "="*80)
            print("⚠️  SIMILAR FILES (From migration backup - review before removing)")
            print("="*80)
        ...
        if report['unique_files']:
            print("\n" + "="*80)
            print("🆕 UNIQUE FILES (Not found in migration backup)")
            print("="*80)
        ...
        print("="*80)
        print(f"1. ✅ Safe to delete: {summary['exact_duplicates']} exact duplicates from migration backup")
        print(f"2. ⚠️  Review carefully: {summary['similar_files']} similar files from migration backup")
        print(f"3. 📝 Manual review needed: {summary['name_only_matches']} name-only matches")
        print(f"4. 🆕 Not from migration backup: {summary['unique_files']} unique files")
        print("\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Analyze Losetests directory against migration backup',
...
This script is SAFE - it only reads files and generates reports.
    ...
    parser.add_argument(
        '--output',
        type=str,
        default='losetests_vs_migration_backup.json',
        help='Output JSON report filename (default: losetests_vs_migration_backup.json)'
    )

    args = parser.parse_args()

    # Run analysis
    analyzer = MigrationBackupAnalyzer(args.base_path)
