#!/usr/bin/env python3
"""
xUnit1051 Error Fix Script

This script automatically fixes xUnit1051 errors by adding TestContext.Current.CancellationToken
to async method calls that accept CancellationToken parameters.

Features:
- Dry-run mode for safe testing
- Simple but effective regex patterns (80/20 approach)
- File-by-file processing with build validation
- Git safety with rollback capability
...
class XUnit1051Fixer:
    def __init__(self, project_root: str, dry_run: bool = True):
        ...
        # Simple but effective regex patterns - targeting 80% of cases
        self.async_method_patterns = [
            # Pattern 1: TestValidateAsync without CancellationToken (most common)
            r'(await\s+\w+\.TestValidateAsync\([^)]*)\)(?!\s*,\s*(?:cancellationToken|TestContext\.Current\.CancellationToken))',
            
            # Pattern 2: ProcessAsync without CancellationToken  
            r'(await\s+\w+\.ProcessAsync\([^)]*)\)(?!\s*,\s*(?:cancellationToken|TestContext\.Current\.CancellationToken))',
            
            # Pattern 3: General async methods ending with single parameter
            r'(await\s+\w+\.\w*Async\(\s*\w+\s*)\)(?!\s*,\s*(?:cancellationToken|TestContext\.Current\.CancellationToken))',
            
            # Pattern 4: Replace CancellationToken.None with TestContext.Current.CancellationToken
            r'CancellationToken\.None',
        ]
        
        self.log_file = self.project_root / f"xunit1051_fix_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        ...
        try:
            ...
            for line in full_output.split('\n'):
                if 'xUnit1051' in line and '.cs(' in line:
                    # Parse error line format: filepath(line,col): error xUnit1051: message
    ...
    def apply_fixes_to_content(self, content: str, filepath: str) -> Tuple[str, int]:
        """Apply fixes to file content and return modified content with fix count"""
        modified_content = content
        fixes_in_file = 0
        
        # Pattern 1-3: Add TestContext.Current.CancellationToken to async methods
        for i, pattern in enumerate(self.async_method_patterns[:3]):
            def replace_func(match):
                original = match.group(0)
                method_call = match.group(1)
                
                # Skip if already has cancellation token
                if 'TestContext.Current.CancellationToken' in original or 'cancellationToken:' in original:
                    return original
                
                # Add the cancellation token as named parameter  
                new_call = f"{method_call}, cancellationToken: TestContext.Current.CancellationToken)"
                self.log(f"  Fix: {original.strip()} -> {new_call.strip()}")
                return new_call
            ...
            if count > 0:
                ...
                fixes_in_file += count
                self.log(f"  Applied {count} fixes for pattern {i+1}")
        
        # Pattern 4: Replace CancellationToken.None
        new_content, count = re.subn(
            self.async_method_patterns[3], 
    ...
    def process_file_safely(self, filepath: str) -> bool:
        """Process a single file safely with validation after each file (from reference script)"""
        ...
        try:
            ...
            # Apply fixes
            modified_content, fixes_in_file = self.apply_fixes_to_content(original_content, str(file_path))
            
            if fixes_in_file == 0:
                self.log(f"  No fixes needed for {file_path.name}")
                return True
            
            if self.dry_run:
                self.log(f"  DRY RUN: Would apply {fixes_in_file} fixes to {file_path.name}")
            ...
            try:
                ...
                self.log(f"  Applied {fixes_in_file} fixes to {file_path.name}")
                
                # CRITICAL: Validate build after EACH FILE (from reference script)
                if self.validate_build(baseline_errors):
                    self.fixes_applied += fixes_in_file
                    backup_path.unlink()  # Remove backup if successful
                    self.log(f"  File validation passed - continuing to next file")
                    return True
                else:
                    self.log(f"  SAFETY ABORT: Build validation failed for {file_path.name}, rolling back", "ERROR")
    ...
    def validate_build(self, baseline_errors: int = None) -> bool:
        """Validate that the build hasn't gotten worse - WITH TRUNCATION DETECTION"""
        if self.dry_run:
            return True
        ...
        if baseline_errors is None:
            # If no baseline provided, just check we didn't break everything
            return current_errors < 1000  # Reasonable upper bound
        
        # CRITICAL SAFETY CHECK: Detect truncated builds (your key insight!)
        ...
        # Normal validation: Allow same number of errors or slight increase
        # Only rollback if errors significantly increased  
        is_acceptable = current_errors <= baseline_errors + 10  # Slightly higher tolerance
    ...
    def run_fixes(self) -> bool:
        ...
        self.log("=== Starting xUnit1051 Error Fix Process ===")
        ...
        # Process each file safely (abort on first failure like reference script)
        successful_files = 0
        for filepath in files_to_process:
            ...
            if self.process_file_safely(filepath):
                ...
                self.log(f"ABORTING: Safety validation failed on {filepath}", "ERROR")
                break  # Stop on first failure for safety
        
        # Summary
        self.log("=== Fix Process Summary ===")
        ...
        if not self.dry_run:
            ...
            if remaining_errors < self.errors_found:
                ...
                self.log(f"Error reduction: {reduction_percent:.1f}%")
                return True
            else:
...
def main():
    parser = argparse.ArgumentParser(description='Fix xUnit1051 errors automatically')
    ...
    print(f"xUnit1051 Error Fixer")
    print(f"Project root: {project_root}")
    print(f"Mode: {'DRY RUN' if dry_run else 'REAL EXECUTION'}")
    print("-" * 50)
    
    fixer = XUnit1051Fixer(project_root, dry_run=dry_run)
